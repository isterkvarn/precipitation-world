...
var player_chunk: Vector3i = Vector3i(0,0,0)
...
func _process(_delta: float) -> void:
	...
	for player: Node3D in get_tree().get_nodes_in_group("player"):
		player_chunk = Vector3i(floor(player.position.x / CHUNK_SIZE), floor(player.position.y / CHUNK_SIZE), floor(player.position.z / CHUNK_SIZE))
